...
<div class="col-sm-9 col-md-9 col-lg-10">
    <section class="mainbody">
        ...
        <form id="formSearch" class='form-inline' action ="<?php echo U('Projects/index');?>" method="get">

            <div class="input-group">
                <input type="text" class="form-control" name='keywords' value="<?php echo ($keywords); ?>" placeholder="搜索projects">
        ...
        <div class="table-responsive">
             <table class="table table-striped table-bordered table-hover">
                ...
                <?php if(is_array($projects)): $i = 0; $__LIST__ = $projects;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        <td><?php echo ($vo["projects_id"]); ?></td>
                        <td><?php echo ($vo["name"]); ?></td>
                        <td><?php echo ($vo["start_time"]); ?></td>
                        <td><?php echo ($vo["end_time"]); ?></td>
                        <td><?php echo (hasImg($vo["thumb"])); ?></td>
                        <td><?php echo (date("Y-m-d H:i",$vo["create_time"])); ?></td>
                        <td><span id="updateStatus" data-id = "<?php echo ($vo["projects_id"]); ?>" data-behavior = "更改状态"  <?php if($vo["status"] == 1): ?>data-status="0" <?php else: ?> data-status="1"<?php endif; ?>><?php echo (status($vo["status"])); ?></span></td>
                        <td><?php echo ($vo["count"]); ?></td>
                        <td>
                            <a href="<?php echo U('Home/Projects/view',array('id'=>$vo[projects_id]));?>" target="_blank"><i class="fa fa-eye"></i> 预览</a>
                            <a id="edit"   class="edit" title="编辑"  data-behavior="编辑" data-id="<?php echo ($vo["projects_id"]); ?>" ><i class="fa fa-edit"></i> 编辑</a>
                            <a id="delete" class="delete" title="删除" data-behavior="删除" data-id="<?php echo ($vo["projects_id"]); ?>" data-status="-1"><i class="fa fa-trash"></i> 删除</a>
                        </td>
...
<script>
    ...
    var SCOPE = {
        'edit_url': "<?php echo U('Projects/edit');?>",
        'add_url': "<?php echo U('Projects/add');?>",
        'set_status_url': "<?php echo U('Projects/setStatus');?>"
